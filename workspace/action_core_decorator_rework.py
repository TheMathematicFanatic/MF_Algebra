...
class Action(MF_Base):
	...
	def __init__(self,
		...
		self.wrap_get_output_expression()
		self.wrap_get_addressmap()
	...
	output_expression_wrappers = [
		'preaddressfunc'
	]
	def wrap_get_output_expression(self):
		func = self.get_output_expression.__func__
		for deco_name in self.output_expression_wrappers:
			decorator = getattr(self, deco_name)
			func = decorator(func)
		self.get_output_expression = lambda *args, **kwargs: func(self, *args, **kwargs)
	...
	def wrap_get_addressmap(self):
		func = self.get_addressmap.__func__
		for deco_name in self.addressmap_wrappers:
			decorator = getattr(self, deco_name)
			func = decorator(func)
		self.get_addressmap = lambda *args, **kwargs: func(self, *args, **kwargs)
