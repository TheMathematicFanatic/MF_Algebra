...
class NewFunctionTest(Scene):
	def construct(self):
		A = Function(
			symbol = '\\alpha',
			symbol_glyph_length = 1,
			algebra_rule_variables = [x,y],
			algebra_rule = x**2/(y**2+1),
		)
		Abc = A(b,c)
		exp = Abc.expand_on_args()
		eq = Abc & exp
		E = Evaluate()
		eq >> E
		self.add(E.mob)
...
class ExpandSumTest(Scene):
	def construct(self):
		S = Sum(n,0,10)
		Sa = S(x**n/fact(n))
		eq = Sa.equation_from_args()
		self.add(eq.mob)
		self.embed()
